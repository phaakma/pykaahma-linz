...
"""
import requests
from pykaahma_linz.CustomErrors import KServerBadRequestError, KServerError, KUnknownItemTypeError
...
class ContentManager:
    ...
    def __init__(self, kserver: "KServer"):
        """
    ...
    def get(self, id: str) -> dict:
        ...
        if not search_result or "error" in search_result:
            raise KServerBadRequestError(f"Content with id {id} not found or invalid request.")
        if len(search_result) == 0:
            return None
        elif len(search_result) > 1:
            raise KServerBadRequestError(f"Multiple contents found for id {id}. Please refine your search.")
        ...
        if item_details.get("kind") == "vector":
            ...
            raise KUnknownItemTypeError(f"Unsupported item kind: {item_details.get('kind')}")
